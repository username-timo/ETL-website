...
const Header: React.FC = () => {
  ...
  return (
    <header
      className={`fixed h-28 top-0 py-0 z-50 w-full overflow-visible transition-all duration-300 ${solidHeader ? "bg-white/97 dark:bg-semidark/97 backdrop-blur-md shadow-md border-b border-gray-200 dark:border-dark_border" : "bg-black/75 backdrop-blur-md shadow-none"}`}
    >
      <div className="container mx-auto lg:max-w-screen-xl md:max-w-screen-md flex items-center justify-between px-4 py-6">
        ...
        <nav className={`hidden lg:flex flex-grow items-center justify-center space-x-6 ${!solidHeader ? '[&_a:not(.lpo-btn)]:!text-white [&_a:not(.lpo-btn)]:hover:!text-secondary' : ''}`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            <svg
              viewBox="0 0 16 16"
              className={`hidden h-6 w-6 dark:block ${!solidHeader && "text-white"}`}
            >
            ...
            <svg
              viewBox="0 0 23 23"
              className={`h-8 w-8 text-dark dark:hidden ${!solidHeader && "text-white"}`}
            >
